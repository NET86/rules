...
| 选定域名消失或移入被包含文件 | 报告结构变化，暂停相关健康观察计数。 |
| V2Fly 抓取失败 | 沿用有效基线，失败日期不计入健康观察次数。 |
| 语音 IP 覆盖骤变 | 保留旧版并报告，其他域名更新继续。 |
| 上游删除且无替代覆盖 | 首次缺失满 14 个日历日、至少 3 个健康 UTC 日期观察确认，且本次抓取健康时退役；失败期间不暂停日历时间。 |
| 上游删除导致单厂商或所属合集关键用例失效 | 保留旧规则并报告。 |
...
| `engines.json` | FlClash 应用及内嵌核心版本；Mihomo 版本见 [下载脚本](../scripts/download_mihomo.py)。 |
| `rules/manifest.json`（仓库根目录下） | 产物、来源、合集成员、格式差异与摘要。 |

Actions 摘要和运行报告列出来源状态与待审项，区分抓取成功、沿用旧版、无有效基线及本地输入；发布成功不代表所有来源都抓取成功。
...
浏览器兼容 HTTPS 依赖仅用于 OpenAI 文档返回 403 时的后备请求。依赖安装失败不会直接阻止生产同步；抓取或解析失败仍报告异常，有有效基线时保留旧版，无有效基线时明确报告不可用。正常抓取且解析通过后才清除此来源异常。CI 必须通过锁定依赖安装和真实传输测试。

语音列表按地址族比较实际覆盖集合：引入新地址族、覆盖超过原来的 4 倍，或保留的旧地址不足一半，均需复核。等价拆分、合并不触发；符合阈值的小量更新继续自动处理。
...
同步与来源检查各维护一个异常 Issue：报告上传后更新，状态不变不重复评论，问题消失后自动关闭。执行结果以 Actions 为准；更新时效仍受调度、来源、CDN 和客户端缓存影响。
